...
LOCATION_DATA_PATH = Path(__file__).with_name("location_data.zip")
...
def load_location_member(location_data, member):
  with location_data.open(member) as location_file:
    return json.load(location_file)


def fallback_location(record, location_data):
  capitals = load_location_member(location_data, "capitals.json")
  fallback_records = (
    capitals["admin1_capitals"].get(f"{record[RECORD_COUNTRY_CODE]}.{record[RECORD_ADMIN1_CODE]}"),
    capitals["country_capitals"].get(record[RECORD_COUNTRY_CODE]),
  )

  for fallback_record in fallback_records:
    if fallback_record is None or fallback_record[RECORD_POPULATION] < MINIMUM_POPULATION:
      continue
    if (
...
def load_location_data():
  location_data = None
  try:
    location_data = zipfile.ZipFile(LOCATION_DATA_PATH)
    metadata = load_location_member(location_data, "metadata.json")
    if metadata.get("schema_version") != LOCATION_DATA_SCHEMA_VERSION:
      raise ValueError(f"Unsupported location data schema: {metadata.get('schema_version')}")
    return location_data
  except (OSError, EOFError, zipfile.BadZipFile, json.JSONDecodeError, ValueError, KeyError) as error:
...
def nearest_record(latitude, longitude, location_data):
  ...
  for key in candidate_grid_keys(latitude, longitude):
    try:
      records = load_location_member(location_data, f"grid/{key.replace(':', '/')}.json")
    except KeyError:
      continue

    for record in records:
      distance_km = calculate_distance_to_point(latitude, longitude, record[RECORD_LATITUDE], record[RECORD_LONGITUDE]) / 1000

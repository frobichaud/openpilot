...
def pair_to_the_pond():
  try:
    ...
    response = requests.post(
      f"{FROGPILOT_API}/pond/pair/request",
      json=payload,
    ...
    if not code:
      return None, "Failed to get pairing code... Please try again!"

    return code, None

  except Exception as exception:
    return None, f"Network error: {exception}"


def poll_status(code, poll_active, max_polls=200, poll_interval=3.0):
  ...
  for _ in range(max_polls):
    ...
    try:
      response = requests.get(
        ...
        params={"api_token": api_token, "code": code, "dongle_id": dongle_id},
        headers={"User-Agent": "frogpilot-api/1.0"},
        timeout=10,
      )
...
def unpair_from_the_pond():
  try:
    ...
    response = requests.post(
      f"{FROGPILOT_API}/pond/pair/unpair",
      json=payload,
      headers={"User-Agent": "frogpilot-api/1.0"},
      timeout=10,
    )

...
from openpilot.selfdrive.controls.lib.longitudinal_mpc_lib.long_mpc import COMFORT_BRAKE, LEAD_DANGER_FACTOR, STOP_DISTANCE, get_safe_obstacle_distance, get_stopped_equivalence_factor, get_jerk_factor, get_T_FOLLOW
...
class FrogPilotFollowing:
  ...
  def update(self, long_control_active, v_ego, sm, frogpilot_toggles):
    ...
    if long_control_active and self.frogpilot_planner.tracking_lead:
      ...
      self.desired_follow_distance = get_safe_obstacle_distance(v_ego, self.t_follow) - get_stopped_equivalence_factor(self.frogpilot_planner.lead_one.vLead)
    else:
      self.desired_follow_distance = 0

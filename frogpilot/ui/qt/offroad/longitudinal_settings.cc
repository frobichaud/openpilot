...
FrogPilotLongitudinalPanel::FrogPilotLongitudinalPanel(FrogPilotSettingsWindow *parent, bool forceOpen) : FrogPilotListWidget(parent), parent(parent) {
  ...
  const std::vector<std::tuple<QString, QString, QString, QString>> longitudinalToggles {
    ...
    {"CalibratedLateralAcceleration", tr("Calibrated Lateral Acceleration"), tr("<b>How hard you corner, learned from your own driving.</b><br><br>The \"Auto\" profile uses this to take curves the way you do. A higher number means carrying more speed through curves. Lower means taking them gentler."), ""},
    {"CalibrationProgress", tr("Calibration Progress"), tr("<b>How much of your own cornering openpilot has learned from.</b><br><br>This only grows while you're the one controlling the speed, so it fills up as you drive curves yourself. At 100% openpilot has seen enough different curves to match the way you take them."), ""},
    {"CurveSpeedProfile", tr("Curve Speed Profile"), tr("<b>How fast openpilot takes curves.</b><br><br>\"Gentle\" and \"Standard\" hold to a fixed, relaxed pace, \"Sport\" corners as hard as your steering has proven it can hold, and \"Auto\" matches the way you take curves yourself."), ""},
    {"MaxLateralAcceleration", tr("Maximum Lateral Acceleration"), tr("<b>The hardest cornering your car's steering has proven it can hold, and the ceiling on every curve speed profile.</b><br><br>It climbs when openpilot gets through a curve with steering effort to spare, and falls back whenever the steering runs out. \"Sport\" aims straight at this number, and the other profiles get pulled down to it any time they ask for more."), ""},
    {"ResetCurveData", tr("Reset Curve Data"), tr("<b>Throw away everything openpilot has learned about how you take curves and start over.</b><br><br>Both \"Auto\" and \"Sport\" go back to their starting values and relearn as you drive. Only available while the car is off."), ""},
    ...
    {"SLCMapboxFiller", tr("Use Mapbox as Fallback"), tr("<b>Fall back to Mapbox for the speed limit when none of your chosen sources have one.</b><br><br>Needs a working internet connection."), ""},
    ...
    if (param == "AdvancedLongitudinalTune") {
      ...
      QObject::connect(resetCurveDataButton, &ButtonControl::clicked, [this]() {
        ...
        if (FrogPilotConfirmationDialog::yesorno(tr("Are you sure you want to completely reset your curvature data?"), this)) {
          ...
          maxLateralAccelerationLabel->setText(QString::number(0.00, 'f', 2) + tr(" m/s²"));
        }
      });

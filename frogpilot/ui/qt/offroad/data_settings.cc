...
namespace {
  bool recordingInProgress(const QDir &directory, const QString &name) {
    return QFile::exists(directory.absoluteFilePath(name + ".lock"));
  }

  bool anyRecordingInProgress(const QDir &directory) {
    return !directory.entryList({"*.mp4.lock"}, QDir::Files).isEmpty();
  }
    ...
    if (id == 0) {
      ...
      if (!selection.isEmpty()) {
        if (ConfirmationDialog::confirm(tr("Delete this screen recording?"), tr("Delete"), this)) {
          std::thread([=]() {
            ...
            bool inProgress = recordingInProgress(recordingsDir, recordingMap[selection]);
            bool success = !inProgress && QFile::remove(recordingsDir.absoluteFilePath(recordingMap[selection]));

            runOnUIThread(screenRecordingsButton, [=]() {
              screenRecordingsButton->setValue(inProgress ? tr("Still recording...") : success ? tr("Deleted!") : tr("Delete failed..."));
            });
      ...
      if (ConfirmationDialog::confirm(tr("Delete all screen recordings?"), tr("Delete All"), this)) {
        std::thread([=]() mutable {
          ...
          bool inProgress = anyRecordingInProgress(recordingsDir);
          bool success = false;
          if (!inProgress) {
      ...
      if (!selection.isEmpty()) {
        ...
        if (!newBase.isEmpty()) {
          ...
          std::thread([=]() {
            ...
            bool inProgress = recordingInProgress(recordingsDir, recordingMap[selection]);
            bool success = !inProgress && QFile::rename(oldPath, newPath);

            runOnUIThread(screenRecordingsButton, [=]() {
              screenRecordingsButton->setValue(inProgress ? tr("Still recording...") : success ? tr("Renamed!") : tr("Rename failed..."));
            });
    ...
    if (id == 0) {
      ...
      if (!name.isEmpty()) {
        ...
        std::thread([=]() {
          ...
          bool success = tarStatus <= 1 && QFileInfo(inProgressPath).size() > 0;
          if (success) {
